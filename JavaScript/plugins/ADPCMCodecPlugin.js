/* Smalltalk from Squeak4.5 with VMMaker 4.20.5 translated as JavaScript source on 4 November 2023 11:12:07 pm */
/* Automatically generated by
	JSPluginCodeGenerator * VMMakerJS-dtl.18 uuid: 544d97d2-2811-46b7-a653-5c1776f146a4
   from
	ADPCMCodecPlugin * VMMaker-dtl.439 uuid: 2c8ef6e2-8460-4120-b973-009902597787
 */
...
primitiveDecodeStereo(count) {
	...
	stepSizeTable = this.pointers[14].storageType === "words" ? this.pointers[14].wordsAsInt16Array() : PrimitiveFailed.signal();
	indexTable = this.pointers[15].storageType === "words" ? this.pointers[15].wordsAsInt16Array() : PrimitiveFailed.signal();
	predictedLeft = predicted[0];
	predictedRight = predicted[1];
	indexLeft = index[0];
	indexRight = index[1];
	for (i = 1; i <= count; i++) {
		if ((UBAND(i, frameSizeMask)) === 1) {
			...
			predictedDeltaRight = predictedDeltaLeft = 0;
			bit = deltaValueHighBit;
			while (bit > 0) {
	...
	predicted[0] = predictedLeft;
	predicted[1] = predictedRight;
	index[0] = indexLeft;
	index[1] = indexRight;
	this.pointers[6] = currentByte;
	this.pointers[7] = bitPosition;
